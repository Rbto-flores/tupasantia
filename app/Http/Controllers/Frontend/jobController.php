...
class jobController extends Controller
{
    ...
    public function create(): View|RedirectResponse
    {
        ...
        if (!$userPlan) {
            Notify::errorNotification('No tienes un plan activo, por favor compra un plan o completa tu perfil para acceder al plan gratis');
            return to_route('company.jobs.index');
        }
        if ($userPlan->job_limit < 1) {
            Notify::errorNotification('Alcanzaste tu límite de publicaciones, por favor actualiza tu plan');
            return to_route('company.jobs.index');
        }
    ...
    public function store(JobCreateRequest $request): RedirectResponse
    {
        ...
        if ($userPlan->job_limit < 1) {
            Notify::errorNotification('You have reached your job limit, please upgrade your plan.');
            return redirect()->back();
        }

        $limits = [
            'featured' => ['key' => 'featured_job_limit', 'message' => 'You have reached your featured job limit, please upgrade your plan.'],
            'highlight' => ['key' => 'highlight_job_limit', 'message' => 'You have reached your highlight job limit, please upgrade your plan.']
        ];

        foreach ($limits as $requestKey => $limit) {
            if ($request->input($requestKey) === "1" && $userPlan->{$limit['key']} < 1) {
                Notify::errorNotification($limit['message']);
                return redirect()->back();
            }
        }
